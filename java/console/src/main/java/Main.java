...
public class Main {
    ...
    public static void main(String[] args) {
        AudioFile audioFile = new AudioFile(Paths.get("audio_hello_world.wav"))
        ...
        disposables.addAll(Arrays.asList(
            ...
            client.getServerEvents()
                ...
                .subscribe(Main::consumeAudioTranscriptDelta, Main::consumeError, Main::onAudioResponseTranscriptCompleted)
        ));
    ...
    public static Mono<Void> sendAudioFileAsync(RealtimeAsyncClient client, AudioFile audioFile) {
                System.out.println("sendAudioFile");

        byte[] audioBytes;
        try {
            audioBytes = audioFile.getAudioFilesBytes();
                        System.out.println("bytes " + audioBytes.length);
